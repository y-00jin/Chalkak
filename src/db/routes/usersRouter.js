...
router.post('/login', async (req, res) => {
    try {
        ...
        if (!isUserExists) {    // 신규 회원 -> 회원가입
            const result = await queries.createUser(userInfo.email, userInfo.user_nm, userInfo.social_type, userInfo.social_id);  // 회원가입
            ...
            res.status(result.result ? 200 : 500).json({ redirectUrl : redirectUrl});
        } else {    // 기존 회원 -> 로그인
            req.session.loginUser = resUsers[0]; // 첫 번째 사용자로 로그인
            res.status(200).json({  redirectUrl: '/memory/new'});
        }
