...
router.get('/active', async (req, res) => {
    ...
    try {
        ...
        activeMemoryInfo = await queries.getMemory(undefined, undefined, loginUser.user_seq_no, undefined, undefined, true);    // 활성화 된 추억 조회
...
router.post('/new', async (req, res) => {
    ...
    try {
        ...
        while (!memoryCodeCheck) {
            ...
            // 추억 코드 중복 확인
            const memoryCodeCheckRes = await queries.getMemoryCodes(undefined, memoryCode);    // 추억 코드 중복 확인
        ...
        // 추억 코드 생성
        const insertMemoryCodeRes = await queries.insertMemoryCode(memoryCode);    // 추억 코드 생성
