...
export default function MemoryChange({ closeEvent }) {
    ...
    useEffect(() => {
        ...
        const getMemorysInfo = async () => {
            try {
                const res = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/api/memories/inactive`);
    ...
    const handleChangeMemory = async () => {
        ...
        await axios.put(`${process.env.REACT_APP_API_BASE_URL}/api/memories/${selectedMemorySeqNo}/active`)
            .then(res => {
                if (res.status === 200) {
    ...
    const ExitMemoryCheck = async (data) => {
        try {
            const res = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/api/memories/memoryCodes/${data.memory_code_seq_no}/users`);
    ...
    useEffect(() => {
        ...
        const ExitMemory = async () => {
            try {
                const res = await axios.delete(`${process.env.REACT_APP_API_BASE_URL}/api/memories/${deleteMemoryInfo.memory_seq_no}`);
                if (res.status === 200) {
